...
class Setup_Material:
    def __init__(self, data, mat_name, Dir, inEngineDir, rootDir):
        ...
        unreal.EditorAssetLibrary.make_directory(self.mat_path)
        unreal.EditorAssetLibrary.make_directory(os.path.join(self.inEngineDir, os.path.basename(self.Dir).split('.')[0],'Textures').replace('\\','/'))
        ...
        matInstancePath = self.find_asset_in_directory() or os.path.join(self.mat_path, 'MI_'+self.mat_name).replace('\\','/')
    ...
    def find_asset_in_directory(self):
        asset_list = unreal.EditorAssetLibrary.list_assets(self.mat_path)
        
        for asset in asset_list:
            if re.match(f'MI_{self.mat_name}_'+r'\d*'+'_TwoSided', str(asset).split('.')[-1]):
                return asset
...
def main():
    ...
    options.share_assets_for_identical_prims = True # Disabling this breaks instancing
